...
class WinXtremeModel extends CI_Model {
	...
	public function InsertUserPredictionn($data)
    {
		$this->db->where('result', null);
		$this->db->where('status', 1)->order_by('prediction_id',"desc")->limit(1);
	    $this->db->from('tbl_prediction');
	    $predictionQuery = $this->db->get();
		$predictionDetails = $predictionQuery->result_array();
		$this->db->where('prediction_id', $predictionDetails[0]['prediction_id']);
		$this->db->where('user_id', $data['user_id']);
		$this->db->where('result', $data['result']);
	    $this->db->from('tbl_matrix');
	    $matrixQuery = $this->db->get();
	    $matrixValue = $matrixQuery->result_array();
	    if($predictionQuery->num_rows() == 1)
	    {
		    if($matrixQuery->num_rows() == 1)
		    {
		    	$oldAmt = $matrixValue[0]['amount'];
				$newAmt = $data['amount'];
				$amt = (float)($oldAmt + $newAmt);
				$data['amount'] = number_format((float)$amt, 2, '.', '');
				$this->db->query("UPDATE `tbl_matrix` SET `amount` = ".$data['amount']." WHERE user_id = ".$data['user_id']." AND result = ".$data['result']." ");
				if ($this->db->affected_rows() == '1')
				{
					$result["status"] = "success";
					$result["statusCode"] = "200";
		    ...
		    {
		    	$amt = $data['amount'];
		    	$data['prediction_id'] = $predictionDetails[0]['prediction_id'];
				$data['amount'] = number_format((float)$amt, 2, '.', '');
				$this->db->insert('tbl_matrix', $data);
				if ($this->db->affected_rows() == '1')
				{
					$result["status"] = "success";
					$result["statusCode"] = "200";
	    ...
	    {
			$result["message"] = "Prediction event not found";
			$result["status"] = "failure";
			$result["statusCode"] = "201";
			return $result;
	    }
	...
	public function getFinalResultt($data)
	{
		$this->db->where('result', null);
		$this->db->where('status', 1)->order_by('prediction_id',"desc")->limit(1);
	    $this->db->from('tbl_prediction');
	    $predictionQuery = $this->db->get();
		$predictionDetails = $predictionQuery->result_array();
		$Pid = $predictionDetails[0]['prediction_id'];
		$this->db->where('prediction_id', $Pid);
		$this->db->select('user_id,result,amount');
	    $this->db->from('tbl_matrix');
	    $query = $this->db->get();
	    $predictionValue = $query->result_array();

		$this->db->where('prediction_id', $Pid);
		$this->db->select('result');
	    $this->db->from('tbl_prediction');
	    $query = $this->db->get();
	    $predictionResult = $query->result_array();

	    if($predictionResult[0]['result'] == null || $predictionResult[0]['result'] == '')
	    {
			$resultwiseAmt  = [];
			$result  = array_column($predictionValue, 'result');
			$amount  = array_column($predictionValue, 'amount');

			$unique_ids = array_unique($result);

			foreach ($unique_ids as $id)
			{
			    $ids = array_keys($result, $id);
			    $finalAmount = array_sum(array_intersect_key($amount, array_combine($ids, $ids)));
				$floatAmount = number_format((float)$finalAmount, 2, '.', '');
			    $resultwiseAmt[] = array("result"=>$id,"amount"=>$floatAmount);
			}
			
			$Camount = array_column($resultwiseAmt, 'amount');
			$resAmount = min($Camount);
			$res = $this->search($resultwiseAmt, 'amount', $resAmount);
			$eventWinColorId = $res[0]['result'];

			$this->db->where('prediction_id', $Pid);
			$this->db->where('result', $eventWinColorId);
			$this->db->select('user_id,amount');
		    $this->db->from('tbl_matrix');
		    $query = $this->db->get();
		    $winningUserResult = $query->result_array();

			$this->db->update('tbl_prediction', array('result'=>$eventWinColorId, 'status'=>0));
			if ($this->db->affected_rows() == '1')
			{
				if(sizeof($winningUserResult) > 0){
					$count = 0;
					for($i = 0; $i < sizeof($winningUserResult); $i++)
					{
						$profitAmount = (float)($winningUserResult[$i]['amount'] * 2);
						$Damount = 18/100;
						$taxAmount = (float)($winningUserResult[$i]['amount'] * $Damount);
						$taxAmount = number_format((float)$taxAmount, 2, '.', '');
						$winningAmount = (float)($profitAmount - $taxAmount);
						$winningAmount = number_format((float)$winningAmount, 2, '.', '');
						$this->db->query("UPDATE `tbl_user` SET `wallet` = wallet + ".$winningAmount." WHERE user_id = ".$winningUserResult[$i]['user_id']." ");
						if ($this->db->affected_rows() == '1')
						{
							$count++;
							$this->db->insert('tbl_transaction_history', array('user_id'=>$winningUserResult[$i]['user_id'], 'type'=>'winning', 'amount'=>$winningAmount, 'transaction_status'=>'success'));
						}
						else
						{
					...
					if (sizeof($winningUserResult) === $count)
					{
	    ...
	    {
			$result2["message"] = "prediction already finished";
			$result2["status"] = "failure";
			$result2["statusCode"] = "201";
			return $result2;
	    }
	...
	public function search($colorwiseAmt, $key, $value)
    {
		$res = [];
	    if (is_array($colorwiseAmt)) {
	        if (isset($colorwiseAmt[$key]) && $colorwiseAmt[$key] == $value) {
	            $res[] = $colorwiseAmt;
	        }

	        foreach ($colorwiseAmt as $subarray) {
	            $res = array_merge($res, $this->search($subarray, $key, $value));
	        }
	    }
	    return $res;
    }
	...
	public function nextPredictionn($data)
	{
    	$key = $data["key"];
    	$data['prediction_event_id'] = 'PE'.$key;
		unset($data['key']);
