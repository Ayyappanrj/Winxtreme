...
class WinXtreme extends CI_Controller {
	...
	public function getFinalResult()
	{
        ...
        $output = $this->WinXtremeModel->getFinalResultt($data);
        echo json_encode($output);
	}
	
	public function tempKey($length)
	{
	...
	{
        header('Access-Control-Allow-Origin: *');
        $data['key'] = $this->tempkey(6);
        $output = $this->WinXtremeModel->nextPredictionn($data);
        echo json_encode($output);
	}

	public function InsertUserPrediction()
    {
        header('Access-Control-Allow-Origin: *');
        $data = $this->input->post();
		$data["user_id"] = $this->session->userdata('user_id');
        $output = $this->WinXtremeModel->InsertUserPredictionn($data);
        echo json_encode($output);
    }
